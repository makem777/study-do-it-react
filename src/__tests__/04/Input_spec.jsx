...
describe('<Input>', () => {
  ...
  it('calls back onChange on input change', () => {
    const changeSub = jest.fn();
    const wrapper = shallow(<Input name="test_name" onChange={changeSub} />);
    expect(changeSub).not.toHaveBeenCalled(); // 호출되지 않은 상태 검증
    const expectedTargetValue = 'update input';
    wrapper.find('input').simulate('change', { target: { value: expectedTargetValue } });
    expect(changeSub).toHaveBeenCalledTimes(1);
    expect(changeSub).toHaveBeenCalledWith('test_name', expectedTargetValue);
  });
});
